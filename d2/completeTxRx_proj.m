...
%%% Channel %%%
carrierFreq        = Flo+freqVin_or1; % Center frequency of the transmission
...
if strcmp(test_type, 'mod')
    ...
    delayed = finddelay(demod_sig,inSig);
    inSig2 = inSig(delayed+1:end-max(0,(ecart-delayed)));
    demod_signal = demod_sig(1:end-max(0,delayed-ecart));
    
    
    % inSig2 = inSig(delayed+1:end-max(0,(ecart-delayed)));
    % demod_signal = demod_sig(1:end-max(0,delayed-ecart));
    [~, BER] = biterr(demod_signal,inSig2);
