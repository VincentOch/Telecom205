function BERexp = BERDFEErr(N0, mod, M , h, N,iteration,nb_err)
...
sprintf('Il y a %d SNR à calculer, pour la DFE', length(N0))
Nc = 31;
...
if (nb_err == 0)
    ...
    H = toeplitzMat(h,L,N);
    [Q,R] = qr(H); %décomposition QR de H
    ...
    parfor jj=1:length(N0)
        I = 0;
        sprintf('calcul du BER correspondant au SNR numéro, via DFE, sans erreurs : %d', jj)
        for ii=1:iter
            ...
            %Calcul des s estimés
            Sest = zeros(size(Z));
            Sest(end) = threshold_detector(Y(end)/R(end,end),mod,M);

            for ll=1:N-1
                ind = N-ll;
                Sest(ind) = threshold_detector((Y(ind)-R(ind,:)*Sest)/R(ind,ind),mod,M);
            end

            %Calcul du BER
            M2 = [];
            for kk=1:mult
                M2 = cat(2,M2,symbols2bits(Sest((kk-1)*long+1:kk*long),mod,M));
            end
...
if (nb_err == 1)
    ...
    parfor jj=1:length(N0)
        I = 0;
        sprintf('calcul du BER correspondant au SNR numéro, via DFE, avec 1 erreur : %d', jj)
        for ii=1:iter
            ...
            M2 = [];
            for kk=1:mult
                c2 = symbols2bits(Sest((kk-1)*long+1:kk*long),mod,M);
...
if (nb_err == 2)
    ...
    parfor jj=1:length(N0)
        I = 0;
        sprintf('calcul du BER correspondant au SNR numéro, via DFE, avec 2 erreurs : %d', jj)
        for ii=1:iter
