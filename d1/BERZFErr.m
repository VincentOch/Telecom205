function BERexp = BERZFErr(N0, mod, M , h, N,iteration,nb_err)
...
sprintf('Il y a %d SNR à calculer, pour la ZF', length(N0))
Nc = 31;
...
if (nb_err == 1)
    ...
    parfor jj=1:length(N0)
        I = 0;
        sprintf('calcul du BER correspondant au SNR numéro, via ZF, avec 1 erreur : %d', jj)
        for ii=1:iter
...
if (nb_err == 2)
    ...
    parfor jj=1:length(N0)
        I = 0;
        sprintf('calcul du BER correspondant au SNR numéro, via ZF, avec 2 erreurs : %d', jj)
        for ii=1:iter
